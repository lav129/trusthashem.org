...
function audioContentType(pathname) {
  ...
  const originReq = https.request(
    ...
    (originRes) => {
      ...
      const out = {
        ...
        "Content-Type": audioContentType(pathname),
        "Cache-Control": req.headers.range
          ? "private, no-store"
          : "public, max-age=86400, immutable",
      };
      if (req.headers.range) out["CDN-Cache-Control"] = "no-store";
