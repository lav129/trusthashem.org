...
function audioContentType(pathname, upstream) {
  ...
  const type = (upstream || "").toLowerCase();
  if (path.endsWith(".ogg")) return "audio/ogg";
  if (path.endsWith(".mp3")) return "audio/mpeg";
  if (path.endsWith(".wav")) return "audio/wav";
  if (path.endsWith(".m4a") || path.endsWith(".mp4")) return "audio/mp4";
  if (type.includes("audio") || type.includes("ogg") || type.includes("mpeg")) return upstream;
...
function originPathname(req) {
  ...
  if (pathname.startsWith("/wp-content/")) return pathname;
  const parts = req.query?.path;
  if (Array.isArray(parts) && parts.length) return `/wp-content/${parts.join("/")}`;
  if (typeof parts === "string" && parts) return `/wp-content/${parts}`;
  return pathname;
}
